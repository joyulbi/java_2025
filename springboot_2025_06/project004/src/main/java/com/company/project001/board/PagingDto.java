...
public class PagingDto {
   private int listtotal;    //#1. 전체 글 123
   private int onepagelist;  //#2. 한 페이지에 보여 줄 게시물의 10
   private int pagetotal;    //#3. 총 페이지수는 123/10 = 13 12페이지 + 3글
   private int bottomlist;   //#4. 하단의 페이지 나누기 
   ...
   public PagingDto(int listtotal, int pstartno) {   
      super();
      this.listtotal   = listtotal;  // 전체 페이지 계산한 값 123
      ...
      this.bottomlist=10;  
      // 1(0)	2(10)	3(20)
      // 현재페이지 : pstartno=10이라면 현재 페이지 번호는 2
      // 현재페이지 : pstartno=20이라면 현재 페이지 번호는 3
